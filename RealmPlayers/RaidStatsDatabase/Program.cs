        ...
        static void Main(string[] args)
        {
            ...
            foreach (string file in luaFiles)
            {
                if(file.EndsWith(".lua") == true)
                {
                        ...
                        if (raidsModified.Count > 0)
                        {
                            ...
                            foreach (var raid in raidsModified)
                            {
                                ...
                                if (raid.RaidOwnerName.ToLower() == "unknown" || raid.RaidOwnerName == "")
                                {
                                    if (rppDatabase == null)
                                    {
                                        string rppDBDir = "C:\\VF_RealmPlayersData\\RPPDatabase\\Database\\";
                            ...
                            Logger.ConsoleWriteLine("--------------------", ConsoleColor.White);
                            VF.Utility.BackupFile(/*"C:\\VF_RealmPlayersData\\RDDatabase\\" + */fightCollectionDatName);
                            VF.Utility.SaveSerialize(/*"C:\\VF_RealmPlayersData\\RDDatabase\\" + */fightCollectionDatName, fights);
                    ...
                    //Utility.BackupFile("C:\\VF_RealmPlayersData\\RDDatabase\\DamageDataSessions\\" + file.Replace(".lua", ".DamageDataSessions.dat"));
                    //Utility.SaveSerialize("C:\\VF_RealmPlayersData\\RDDatabase\\DamageDataSessions\\" + file.Replace(".lua", ".DamageDataSessions.dat"), dataSessions);
                }
            }
