...
namespace VF
{
    public class SQLMigration
    {
        ...
        public static int LoadPlayer(string _PlayerName, WowRealm _Realm, SQLPlayerID _PlayerID /*= SQLPlayerID.Invalid()*/, out Player _ResultPlayer)
        {
            PlayerHistory _ResultPlayerHistory;
            ExtraData _ResultExtraData;
            using (var conn = new NpgsqlConnection(SQLComm.g_ConnectionString))
            {
                return VF.SQLMigration.LoadFullPlayer(conn, _PlayerName, _Realm, _PlayerID, out _ResultPlayerHistory, out _ResultPlayer, out _ResultExtraData);
            }
        }
        public static int LoadPlayer(string _PlayerName, WowRealm _Realm, SQLPlayerID _PlayerID /*= SQLPlayerID.Invalid()*/, out ExtraData _ResultExtraData)
        {
        ...
        public static int LoadPlayer(string _PlayerName, WowRealm _Realm, SQLPlayerID _PlayerID /*= SQLPlayerID.Invalid()*/, out PlayerHistory _ResultPlayerHistory)
        {
            Player _ResultPlayer;
            ExtraData _ResultExtraData;
            using (var conn = new NpgsqlConnection(SQLComm.g_ConnectionString))
        ...
        public static int LoadPlayer(string _PlayerName, WowRealm _Realm, SQLPlayerID _PlayerID /*= SQLPlayerID.Invalid()*/, out PlayerHistory _ResultPlayerHistory, out Player _ResultPlayer, out ExtraData _ResultExtraData)
        {
            using (var conn = new NpgsqlConnection(SQLComm.g_ConnectionString))
            {
                return VF.SQLMigration.LoadFullPlayer(conn, _PlayerName, _Realm, _PlayerID, out _ResultPlayerHistory, out _ResultPlayer, out _ResultExtraData);
        ...
        public static int LoadFullPlayer(NpgsqlConnection _Connection, string _PlayerName, WowRealm _Realm, SQLPlayerID _PlayerID /*= SQLPlayerID.Invalid()*/, out PlayerHistory _ResultPlayerHistory, out Player _ResultPlayer, out ExtraData _ResultExtraData)
        {
            int playerUpdateCount = 0;
            _ResultPlayerHistory = new PlayerHistory();
            _ResultPlayer = new Player();
            _ResultExtraData = new ExtraData();
            ...
            if(_PlayerID.IsValid() == false)
            {
                ...
                if (sqlComm.GetPlayerID(_Realm, _PlayerName, out _PlayerID) == false || _PlayerID.IsValid() == false)
                    return -1;
            }
            ...
            _Connection.Open();
            using (var reader = _Connection.BeginBinaryExport("COPY (SELECT upload.id, upload.uploadtime, upload.contributor, character.updatetime, character.race, character.class, character.sex, character.level" +
                ...
                " WHERE character.playerid = " + _PlayerID.ID + " ORDER BY character.updatetime) TO STDIN BINARY"))
            {
            ...
            if (uniqueGearItems.Count > 0)
            {
                ...
                if (sqlSearchIds != "" && sqlSearchIds != ", ")
                {
                    ...
                    {
                        _Connection.Open();
                        using (var reader = _Connection.BeginBinaryExport("COPY (SELECT id, itemid, enchantid, suffixid, uniqueid FROM ingameitemtable WHERE id IN (" + sqlSearchIds + ")) TO STDIN BINARY"))
                        {
                    ...
                    for (int gI = 0; gI < allGearItems.Count; ++gI)
                    {
                        ...
                        {
                            _Connection.Open();
                            using (var reader = _Connection.BeginBinaryExport("COPY(SELECT itemslot, gemid1, gemid2, gemid3, gemid4 FROM PlayerGearGemsTable WHERE gearid=" + gearID + ") TO STDIN BINARY"))
                            {
                                while (reader.StartRow() != -1)
                                {
            ...
            if (uniqueArenaIDs.Count > 0 && (uniqueArenaIDs.Count > 1 || uniqueArenaIDs[0] != 0))
            {
                ...
                _Connection.Open();
                using (var reader = _Connection.BeginBinaryExport("COPY (SELECT id, teamname, teamrating, gamesplayed, gameswon, playergamesplayed, playerrating FROM PlayerArenaDataTable WHERE id IN (" + sqlSearchIds + ")) TO STDIN BINARY"))
                {
                    while (reader.StartRow() != -1)
                    {
            ...
            {
                _Connection.Open();
                using (var reader = _Connection.BeginBinaryExport("COPY (SELECT upload.contributor, playerseen.updatetime, mount.name FROM playermounttable playerseen" +
                    ...
                    " WHERE playerseen.playerid = " + _PlayerID.ID + ") TO STDIN BINARY"))
                {
                    while (reader.StartRow() != -1)
                    {
                        int contributorID = reader.Read<int>(NpgsqlDbType.Integer);
                ...
                _Connection.Close();
                _Connection.Open();
                using (var reader = _Connection.BeginBinaryExport("COPY (SELECT upload.contributor, playerseen.updatetime, pet.name, pet.level, pet.creaturefamily, pet.creaturetype FROM playerpettable playerseen" +
                ...
                _Connection.Close();
                _Connection.Open();
                using (var reader = _Connection.BeginBinaryExport("COPY (SELECT upload.contributor, playerseen.updatetime, companion.name, companion.level FROM playercompaniontable playerseen" +
