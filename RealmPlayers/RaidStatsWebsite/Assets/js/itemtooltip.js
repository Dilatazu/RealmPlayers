...
function () {
	...
	function b(W, S, X, Q, T) {
		...
		var R = "";
		for (var U in T) {
		    if (U != "rand" && U != "ench" && U != "gems" && U != "sock" && U != "pcs") {
				continue
			}
	...
	function l(P, R, Q) {//Normalt sett buggad funktion
        ...
        3850)
	    if (e == P && D == R && H == Q) {//om detta objektet är det som försöker visas i tooltip just nu
			N(LANG.tooltip_loading);
		}
	}
	function A(P, R, Q) {
	...
	this.register = function (Q, S, R, P) { //Q == 3 (global v(item=3, npc, etc etc)), S == itemID, R == 0?, P == tooltip stuffs json
		var O = p[Q][0];
		k(Q, S, R);//säkerställ att g_items objectet existerar, annars skapa
		if (O[S].timer) {
		...
		if (e == Q && S == D && H == R) {//om detta objektet är det som försöker visas i tooltip just nu
			N(O[S][I(R)], O[S].icon)
		}
	};
	...
	_link.href = "/assets/power.css?version=1.3";
