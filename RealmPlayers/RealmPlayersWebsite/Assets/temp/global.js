...
function g_createHeader(c) {
	...
	if (c != null && c >= 0) {
		...
		// Tools
		case 1:
			Menu.addButtons(b, Menu.explode(mn_tools));
			break;
		// Forums
		case 2:
		...
		// More
		/* case 2:
			Menu.addButtons(b, Menu.explode(mn_server));
			break;
		// Bugtracker
		case 4:
			Menu.addButtons(b, Menu.explode(mn_bugtracker));
			break;
		// Armory
		case 5:
			pr_initTopBarSearch();
		...
		// Mods' Retreat
		case 11:
			Menu.addButtons(b, Menu.explode(mn_modsretreat));
			break;
		// Editors' Lounge
		case 12:
			Menu.addButtons(b, Menu.explode(mn_editorslounge));
			break;
		// Devs' Corner
		case 13:
			Menu.addButtons(b, Menu.explode(mn_devscorner));
			break; */
		}
	} else {
...
function g_createProgressBar(c) {
	...
	if (c.bgcolor) {
		e = ce("div");
		e.className = "progressbar-" + c.bgcolor;
		e.style.width = (100 - c.width) + "%";
		e.style.styleFloat = e.style.cssFloat = "right";
		e.style.borderLeft = "none!important";
		if (c.rightText) {
			e.style.textAlign = "center"
		}
		ae(e, ct(c.rightText ? c.rightText : String.fromCharCode(160)));
		ae(d, e)
	}
	...
	if (c.bgcolor) {
		e.style.borderRight = "none!important"
	}
	ae(e, ct(c.leftText ? c.leftText : String.fromCharCode(160)));
	ae(d, e);
	return d
}
...
function g_getIngameLink(b, e, c) {
	var col = g_pageInfo.color;
	prompt(LANG.prompt_ingamelink,'/script DEFAULT_CHAT_FRAME:AddMessage("\\124c'+q_colors[g_pageInfo.color]+"\\124Hitem:"+g_pageInfo.typeId+":0:0:0:0:0:0:0:0\\124h["+g_pageInfo.name.replace(/"/g, '\\"')+']\\124h\\124r");')
};
function g_getForumLink(b, e, c) {
	var col = q_colors[g_pageInfo.color];
	var colour = col.replace("ff", "");
	prompt(LANG.prompt_ingamelink,"[url='http://www.aowow.org/?item="+g_pageInfo.typeId+"'][color=#"+colour+"]"+g_pageInfo.name+"[/color][/url]")
};
...
function ss_appendSticky() {
	...
	ss_array = lv_screenshots.concat(wh_screenshots);
	var k = in_array(ss_array, 1, function (a) {
		return a.sticky
	});
	if (k != -1) {
		var c = ss_array[k];
		...
		if (g_user.id > 0) {
			...
			l.href = "?account=signin" + "&next=" + g_types[g_pageInfo.type] + "=" + g_pageInfo.typeId;
		}
		if (ss_conf ==3)
					{
					l.style.color="grey";
					...
					g_addTooltip(l,LANG.lvscreenshot_submit_disabled);
					l.href = "javascript:;";
					}
		...
		if(wh_ss == 'load'){
			ae(l, ct(sprintf(LANG.infobox_showall, ss_array.length / 2)));
			ssn.innerHTML = " (" + ss_array.length / 2 + ")";
		}
		else{
			ae(l, ct(sprintf(LANG.infobox_showall, ss_array.length)));
			ssn.innerHTML = " (" + ss_array.length + ")";
		}
		ae(n, l);
		ae(g, n);
		ae(m, g)
	} else {
		if(ss_conf==3){pip=' style="cursor:default!important;text-decoration:none!important;color:grey!important;" '}else{pip=" "}
		var l;
		if (g_user.id > 0) {
			l = '<a'+pip+'href="javascript:;" onclick="ss_submitAScreenshot(); return false">'
		} else {
			l = '<a'+pip+'href='+(ss_conf!=3 ? "?account=signin" + "&next=" + g_types[g_pageInfo.type] + "=" + g_pageInfo.typeId : "javascript:;")+'>'
		}
...
function Ajax(b, c) {
	...
	a.addEventListener("progress", onProgress, false);
...
function onProgress(evt) {
  if (evt.lengthComputable) {
    var ploaded = evt.loaded / evt.total;
  } else {
	...
	showDepth: function (M, c, C, B, N, F, z, v) {
		...
		for (var L = 0, t = c.length; L < t; ++L) {
			...
			if (Q[0] == null && Q[2] == null) {
				...
				if(Q[0] == null && Q[2] != null){
					//U.href = Q[2];
					G.className = "separator-link";
					U.classname = "separator-link";
				}
				if (Q.checked) {
				...
				if (Q.tinyIcon) {
					S.style.background = "url(images/icons/small/" + Q.tinyIcon.toLowerCase() + ".gif) left center no-repeat"
				} else {
					if (Q.socketColor) {
...
var Icon = {
	...
	sizes2: [18, 36, 56],
	create: function (c, k, h, b, r, t, q, e, j, a) {
		var g = ce("div"),
		d = ce("ins"),
		f = ce("del");
		//if (a) {
		//	g.onclick = aowow.account.recent(c)}
		if(t) {g_addTooltip(g,t)}
		if (k == null) {
			k = 1
		}
		g.className = "icon" + Icon.sizes[k];
		if (q != null)
		{g.className += " icon" + Icon.sizes[k] + "-" + q;}
		ae(g, d);
		ae(g, f);
...
var Tooltip = {
	...
	fix: function (d, b, f, i, t) {
		...
		//if(t == 'item'){
		//	d.style.background = "url(http://static.wowhead.com/modelviewer/thumbs/item/" + i + ".png) 90% 5px no-repeat";
		//}
		//if(t == 'npc'){
//			d.style.background = "url(http://static.wowhead.com/modelviewer/thumbs/npc/" + i + ".png) 90% 5px no-repeat";
//		}
	},
	...
	showAtXY: function (d, a, e, c, b, m, t) {
		if (Tooltip.disabled) {
			return
		}
		Tooltip.prepare();
	...
	createCenteredIcons: function (h, c, p, m) {
		if (h != null) {
			...
			for (var e = 0, j = h.length; e < j; ++e) {
				...
				if(h.length == 10){
					n = ce("div");
					ae(n, ct(g_items[b].name_enus));
					ae(l, n);
				}
				//ae(o, ct(g_items[b].name_enus))
	...
	coDetach: function (a) {
		if (a.replyTo == 0) {
			...
			if (confirm(LANG.confirm_detachcomment)) {
				...
				//a.parentDiv.style.padding = "6px 6px 0!important";
				a.replyTo = 0;
...
Listview.templates = {
	...
	item: {
		...
		}/* ,
		{
			...
			compute: function (d, e) {
				...
				var b = ce("a");
				var c = Listview.funcBox.getItemType(d.classs, d.subclass, d.subsubclass);
				b.href = "http://www.wowhead.com/?item="+d.id";
				ae(b, ct(c.text));
				ae(e, b)
			}
		} */],
	...
	itemset: {
		...
		},
		{
			...
			compute: function (a, b) {
				...
				if (b)
				{
				b.className="c"+a.classes;
				return g_itemset_classes[a.classes]
				}
			},
			sortFunc: function (d, c, e) {
				return strcmp(d.classes, c.classes, g_itemset_classes)
			}
			// Original code for itemset_classes, currently not used
	...
	npc: {
		...
		columns: [{
			...
			value: "name",
			compute: function (c, f) {
				if(c.boss){f.className="icon-boss-padded"}
				var b = ce("a");
				b.style.fontFamily = "Verdana, sans-serif";
				b.href = this.template.getItemLink(c);
				ae(b, ct(c.name));
		...
		},
		{
			...
			compute: function (a, c) {
				if (a.side) {
					...
					ae(c, b)
					b.onmouseover=function(d){Tooltip.showAtCursor(d,g_sides_tp[a.side],0,0,"q")};
		...
		},
		{
			...
			compute: function (a, b) {
				if (a.minlevel > 0 && a.maxlevel > 0) {
					if (a.minlevel != a.maxlevel) {
						l = ce("span");
						m = a.minlevel + LANG.hyphen + a.maxlevel;
						ae(l, ct(m));
						if(a.reqlvl){
							r = ce("span");
							r.className = "q0";
							ae(r, ct(' (' + a.reqlvl + ')'));
							g_addTooltip(r, LANG.tooltip_reqlevel);
							ae(l, r);
						}
						ae(b, l);
						
						//return a.minlevel + LANG.hyphen + a.maxlevel + ' (' + a.reqlvl + ')0'
					} else {
						return a.minlevel
					}
				}
	...
	comment: {
		...
		compute: function (t, H) {
			...
			if (D) {
				...
				j.href = K.href = "?account=signin" + "&next=" + g_types[g_pageInfo.type] + "=" + g_pageInfo.typeId;
			}
			ae(j, ct("[+]"));
			...
			var G = ce("a");
			G.href = (t.wowhead==1 ? "http://www.wowhead.com/user=" : "?user=") + t.user;
			ae(G, ct(t.user));
			ae(w, G);
			ae(w, ct(" "));
			var a = ce("a");
			...
			if (J && t.wowhead != 1) {
				...
				if (g_user.id > 0) {
					...
					f.href = "?account=signin" + "&next=" + g_types[g_pageInfo.type] + "=" + g_pageInfo.typeId;
				}
				ae(u, f);
				ae(u, ct("|"));
				ae(k, u)
			...
			if (g_user.id > 0) {
				...
				h.href = "?account=signin" + "&next=" + g_types[g_pageInfo.type] + "=" + g_pageInfo.typeId;
			}
			ae(k, h);
			}
			if (I) {
		...
		createNote: function (g) {
			...
			if (g_user.id > 0) {
				...
				b.href = "?account=signin" + "&next=" + g_types[g_pageInfo.type] + "=" + g_pageInfo.typeId;
			}
		...
		onNoData: function (c) {
			if (typeof g_pageInfo == "object" && g_pageInfo.type > 0) {
				...
				if (g_user.id > 0) {
					...
					b = b.replace("<a>", '<a href="?account=signin' + "&next=" + g_types[g_pageInfo.type] + "=" + g_pageInfo.typeId + '">');
			...
			compute: function (j, i) {
				...
				if (e || f) {
					...
					if (f) {
						...
						var b = ce("a");
						b.href = (j.wowhead==1 ? "http://www.wowhead.com/user=" : "?user=") + j.user;
						ae(b, ct(j.user));
						ae(g, b);
						if (e) {
							ae(g, ct(LANG.hyphen))
	...
	screenshot: {
		...
		compute: function (k, e, l) {
			...
			if (k.wowhead==1){e.className = "screenshot-cell wowhead";}
			e.vAlign = "bottom";
			var q = ce("a");
			...
			var a = (k.approved == 0);
				...
				a = ce("a");
				a.href = "javascript:;";
				a.onclick = new Ajax("/?screenshot=approve&id="+k.id+"");
				a.className = "approve-icon";
			...
			ae(p, j);
			ae(e, p);
			if (!k.wowhead || g_user.name == k.user && !k.wowhead)
				{
				ae(p, ce("br"))
				}
			if (g_user.roles == 2 && !k.wowhead)
			{
				var z = (k.approved == 0);
				if (z) 
				{
				a = ce("a");
				a.href = "?screenshot=approve&id="+k.id;
				a.className = "approve-icon";
				g_addTooltip(a,LANG.lvscreenshot_approve)
				//a.onclick = new Ajax("?screenshot=approve&id="+k.id);
				ae(a, ct("ap"));
				ae(p, a);
				ae(p, ct(" "))
			...
			if (g_user.roles == 2 && !k.wowhead || g_user.name == k.user && !k.wowhead)
			{
				if (g_user.roles == 2 || k.approved ==0)
				{
				a = ce("a");
				a.href = "?screenshot=del&id="+k.id;
				//a.onclick = aowow.screenshot.del(k.id);
				a.className = "delete-icon";
				g_addTooltip(a,LANG.lvscreenshot_delete)
				ae(a, ct("de"));
				ae(p, a);
				ae(p, ct(" "))
				}
			...
			if (g_user.roles == 2 && !k.wowhead)
			{
				...
				{
				s = ce("a");
				s.href = "?screenshot=stick&id="+k.id+"&type="+k.type+"."+k.typeid;
				s.className = "stick-icon";
				g_addTooltip(s,LANG.lvscreenshot_stick)
				ae(s, ct("st"));
				ae(p, s);
				ae(p, ct(" "))
				}
			...
			p.style.height = "1em";
			if (g_user.roles == 2 && g_getLocale(true) == 0 && k.caption && !k.wowhead || g_user.name == k.user && k.approved ==0 && g_getLocale(true) == 0 && k.caption && !k.wowhead)
			{
		...
		createNote: function (d) {
			if (typeof g_pageInfo == "object" && g_pageInfo.type > 0) {
				...
				if (g_user.id > 0) {
					...
					g_addTooltip(b,LANG.lvscreenshot_submit_disabled);
					b.href = "javascript:;";
					}
				} else {
					if (ss_conf ==3)
		...
		onNoData: function (c) {
			if (typeof g_pageInfo == "object" && g_pageInfo.type > 0) {
				var a = "<b>" + (wh_ss == 'load' ? LANG.lvnodata_ss4 : (ss_conf ==3 ? LANG.lvnodata_ss1 : LANG.lvnodata_ss1)) + '</b><div class="pad2"></div>';
				if (ss_conf!=3)
				{
				if (g_user.id > 0) {
				...
				{
				var b = (wh_ss == 'load' ? LANG.lvnodata_ss5 : LANG.lvnodata_ss5);
				a += b
				}
				c.style.padding = "1.5em 0";
				c.innerHTML = a
	...
	profile: {
		...
		columns: [{
			...
			compute: function (f, c, h) {
				...
				if (f.guild) {
					var k = ce("a");
					k.className = "q1";
					k.href = "?guild=" + f.region + "." + f.realm + "&filter=cr=9;crs=0;crv=" + str_replace(urlencode(f.guild), "%20", "+") + "&roster=1";
					ae(k, ct(f.guild));
					ae(g, ct("<"));
					ae(g, k);
					ae(g, ct(">"))
		...
		},
		{
			...
			compute: function (a, f) {
				...
				if(a.race == 2 || a.race == 5 || a.race == 6 || a.race == 8 || a.race == 10)
				{
					a.faction = 1;
				}
				else
...
Menu.fixUrls(mn_characters, "?characters=");
var g_dev = false;
var g_locale = {
	id: 0,
	name: "enus"
...
g_items.createIcon = function (d, b, a, c) {
	return Icon.create(g_items.getIcon(d), b, null, "?item=" + d, a, c, null, a)
};
...
*/
		/* P = aa.href.match(/()\?(item|quest|spell|achievement|npc|object|profile)=([^&#]+)/) */
		P = aa.href.match(/()\?(item|quest|spell|npc|object)=([^&#]+)/)
		//TODO: Wowpedia Tips
		/* if (P == null)
			{
				P = aa.href.match(/http:\/\/(.+?)?\.?wowpedia\.org([^&#]+)/)
			}	 */	
		if (P == null && aa.rel) {
			T = 0;
			S = 1;
...
/* 			P = aa.rel.match(/(item|quest|spell|achievement|npc|object|profile).?([^&#]+)/) */
			P = aa.rel.match(/(item|quest|spell|npc|object).?([^&#]+)/)
		}
		if (aa.rel) {
	...
	function b(W, S, X, Q, T) {
		...
		var R = "";
		for (var U in T) {
			if (U != "rand" && U != "ench" && U != "gems" && U != "sock" && U != "wowhead" && U != "wowpedia" && U != "aowow") {
				continue
			}
		...
		*/
		...
		if (R == "&wowhead=true")
		{
			P = "http://" + g_getDomainFromLocale(g_locale.id) + ".wowhead.com";
			P += "/" + p[W][1] + "=" + S + "&power";
			//P = "http://www.wowpedia.org/index.php?title=Sea_Turtle_(mount)&action=render";
		}
		//TODO: Wowpedia Tips
		/* if (R == "&wowpedia=true")
		{
			//P = 
			P = "http://www.wowhead.com/item=57191&power";
		} */
	...
	function N(R, S, M, t) {
		...
		if (!R) {
			...
			if (q != null) {
				...
				if (q.ench) {
					R = R.replace('<!--en:12345-->', '<span class="c1">lold FUCK</span>');
					//R = R.replace(new RegExp('<span class="c(' + q.c + ')" style="display: none">(.+?)</span>', "g"), '<span class="c$1">$2</span><br />')
				}
				if (q.lvl) {
		...
		if (g == 1) {
			...
			Tooltip.setIcon(S);
			Tooltip.showAtXY(R, B, z, o, x, M, t);
		}
	}
	function l(P, R, Q) {
		if (e == P && D == R && H == Q) {
	...
	function d(P, O) {
		return P + (O.rand ? "r" + O.rand: "") + (O.gems ? "g" + O.gems.join(",") : "") + (O.sock ? "s": "")
	}
	this.register = function (Q, S, R, P, T) {
		var O = p[Q][0];
		k(Q, S, R);
		if (O[S].timer) {
...
function () {
	...
	function hide(e){if(e&&!g_isLeftClick(e)){return}if(container){container.style.display="none"}}
...
function pr_getSpecFromTalents(b, e) {
	...
	if (e == null || (e[0] == 0 && e[1] == 0 && e[2] == 0)) {
		a = -1
	} else { if (e[0] - e[1] > 5 && e[0] - e[2] > 5) {
			a = 1
		} else { if (e[1] - e[0] > 5 && e[1] - e[2] > 5) {
				a = 2
			} else { if (e[2] - e[0] > 5 && e[2] - e[1] > 5) {
					a = 3
				}
			}
		}
	...
	if (a <= 0) {
		...
		d = g_chr_specs[b][a - 1];
		c = "classes/trees/" + g_file_classes[b] + "_" + a
	}
	return {
		id: a,
		name: d,
		icon: "images/talent/" + c + ".gif"
	}
};
function g_trackEvent(d,a,c,e){function b(){{pageTracker._trackEvent(d,a,c,e)}}$(document).ready(b)}
var U_GROUP_TESTER=1;var U_GROUP_ADMIN=2;var U_GROUP_EDITOR=4;var U_GROUP_MOD=8;var U_GROUP_BUREAU=16;var U_GROUP_DEV=32;var U_GROUP_VIP=64;var U_GROUP_BLOGGER=128;var U_GROUP_PREMIUM=256;var U_GROUP_LOCALIZER=512;var U_GROUP_SALESAGENT=1024;var U_GROUP_SCREENSHOT=2048;var U_GROUP_VIDEO=4096;var U_GROUP_APIONLY=8192;var U_GROUP_PENDING=16384;
function g_cookiesEnabled(){document.cookie="enabledTest";return(document.cookie.indexOf("enabledTest")!=-1)?true:false}
function g_getCookie(a){if(g_user.id>0){return g_user.cookies[a]}else{return gc(a)}}function g_setCookie(c,d,b){var a=c.substr(0,5)=="temp_";if(!b&&g_user.id>0&&!a){new Ajax("/cookie="+c+"&"+c+"="+urlencode(d),{method:"get",onSuccess:function(e){if(e.responseText==0){g_user.cookies[c]=d}}})}else{if(b||g_user.id==0){sc(c,14,d,null,"aowow")}}}
function g_addTooltip(b,c,a){if(!a&&c.indexOf("<table>")==-1){a="q"}b.onmouseover=function(d){Tooltip.showAtCursor(d,c,0,0,a)};b.onmousemove=Tooltip.cursorUpdate;b.onmouseout=Tooltip.hide}
var Announcement=function(a){if(!a){a={}}cO(this,a);if(this.parent){this.parentDiv=ge(this.parent)}else{return}if(g_user.id==0&&(!g_cookiesEnabled()||g_getCookie("announcement-"+this.id)=="closed")){return}this.initialize()};Announcement.prototype={initialize:function(){this.parentDiv.style.display="none";if(this.mode===undefined||this.mode==1){this.parentDiv.className="announcement announcement-contenttop"}else{this.parentDiv.className="announcement announcement-pagetop"}var f=this.innerDiv=ce("div");f.className="announcement-inner text";this.style=this.style.replace("%tb%",(""));this.setStyle(this.style);var b=null;var e=parseInt(this.id);if(e>=0){b=ce("a");{ae(b,ct("[X]"))}b.id="closeannouncement";b.href="javascript:;";b.className="announcement-close";if(this.nocookie){b.onclick=this.hide.bind(this)}else{b.onclick=this.markRead.bind(this)}ae(f,b);g_addTooltip(b,LANG.close)}if(g_user&&(g_user.roles&(U_GROUP_ADMIN|U_GROUP_BUREAU))>0&&Math.abs(e)>0){if(e<0){b=ce("a");b.style.cssFloat=b.style.styleFloat="right";b.href="/admin=announcements&id="+Math.abs(e)+"&status=2";b.onclick=function(){return confirm("Are you sure you want to delete "+this.name+"?")};ae(b,ct("Delete"));var d=ce("small");ae(d,b);ae(f,d);b=ce("a");b.style.cssFloat=b.style.styleFloat="right";b.style.marginRight="10px";b.href="/admin=announcements&id="+Math.abs(e)+"&status="+(this.status==1?0:1);b.onclick=function(){return confirm("Are you sure you want to delete "+this.name+"?")};ae(b,ct((this.status==1?"Disable":"Enable")));var d=ce("small");ae(d,b);ae(f,d)}b=ce("a");b.style.cssFloat=b.style.styleFloat="right";b.style.marginRight="10px";b.href="/admin=announcements&id="+Math.abs(e)+"&edit";ae(b,ct("Edit announcement"));var d=ce("small");ae(d,b);ae(f,d)}var c=ce("div");c.id=this.parent+"-markup";ae(f,c);ae(this.parentDiv,f);ae(f,ce("div",{style:{clear:"both"}}));this.setText(this.text);setTimeout(this.show.bind(this),1000)},show:function(){this.parentDiv.style.display=""},hide:function(){$fx(this.parentDiv).fxAdd({type: 'opacity', to: 50, step: 10, delay: 30, onfinish:this.parentDiv.style.display="none"}).fxRun();},markRead:function(){g_setCookie("announcement-"+this.id,"closed");this.hide()},setStyle:function(a){this.style=a;this.innerDiv.setAttribute("style",a)},setText:function(a){this.text=a;Markup.printHtml(this.text,this.parent+"-markup")}};
function pr_getGearScoreQuality(h,e,g,c,a){var f,d,b=0;if(!e){return 0}if(c==null){b=12.25390625;if(h<55){if(g){b-=81/256}b-=18/16;if(h<25){b-=25/16;if(h<20){b-=30/16}}}}else{switch(c){case 1:case 5:case 7:case 16:case 17:b=1;break;case 3:case 10:case 6:case 8:b=3/4;break;case 9:case 2:case 15:case 11:case 12:case 13:case 14:b=9/16;break;case 18:b=81/256;break}if(a){b*=2}}if(h>80){f=((h-80)*30.5)+200}else{if(h>=70){f=((h-70)*9.5)+105}else{if(h>=60){f=((h-60)*4)+65}else{f=h+5}}}f=Math.ceil(f*b);if(e>=f/0.9){d=5}else{d=4;while(e<f&&d>0){f*=0.9;d--}}return d}
Dialog=function(){var d=this,i,r=null,m,j={},l,c=false,o=ce("form"),b={};o.onsubmit=function(){p();return false};this.show=function(u,v){if(u){m=u;i=Dialog.templates[m];d.template=i}else{return}if(i.onInit&&!c){(i.onInit.bind(d,o,v))()}if(v.onBeforeShow){j.onBeforeShow=v.onBeforeShow.bind(d,o)}if(i.onBeforeShow){i.onBeforeShow=i.onBeforeShow.bind(d,o)}if(v.onShow){j.onShow=v.onShow.bind(d,o)}if(i.onShow){i.onShow=i.onShow.bind(d,o)}if(v.onHide){j.onHide=v.onHide.bind(d,o)}if(i.onHide){i.onHide=i.onHide.bind(d,o)}if(v.onSubmit){j.onSubmit=v.onSubmit}if(i.onSubmit){r=i.onSubmit.bind(d,o)}if(v.data){c=false;l={};cO(l,v.data)}d.data=l;Lightbox.show("dialog-"+m,{onShow:a,onHide:k})};this.getValue=function(u){return h(u)};this.setValue=function(v,u){f(v,u)};this.getSelectedValue=function(u){return q(u)};this.getCheckedValue=function(u){return t(u)};function a(u,v){if(v||!c){e(u)}if(i.onBeforeShow){i.onBeforeShow()}if(j.onBeforeShow){j.onBeforeShow()}Lightbox.setSize(i.width,i.height);u.className="dialog";n();if(i.onShow){i.onShow()}if(j.onShow){j.onShow()}}function e(E){ee(E);ee(o);var u=ce("div");u.className="text";ae(E,u);ae(u,o);if(i.title){var N=ce("h1");ae(N,ct(i.title));ae(o,N)}var G=ce("table"),C=ce("tbody"),z=false;ae(G,C);ae(o,G);for(var M=0,O=i.fields.length;M<O;++M){var v=i.fields[M],D;if(!z){tr=ce("tr");th=ce("th");td=ce("td")}v.__tr=tr;if(l[v.id]==null){l[v.id]=(v.value?v.value:"")}var B;if(v.options){B=[];if(v.optorder){cO(B,v.optorder)}else{for(var L in v.options){B.push(L)}}if(v.sort){B.sort(function(T,S){return v.sort*strcmp(v.options[T],v.options[S])})}}switch(v.type){case"caption":th.colSpan=2;th.style.textAlign="left";th.style.padding=0;if(v.compute){(v.compute.bind(d,null,l[v.id],o,th,tr))()}else{if(v.label){ae(th,ct(v.label))}}ae(tr,th);ae(C,tr);continue;break;case"textarea":var P=D=ce("textarea");P.name=v.id;if(v.disabled){P.disabled=true}P.rows=v.size[0];P.cols=v.size[1];td.colSpan=2;if(v.label){th.colSpan=2;th.style.textAlign="left";th.style.padding=0;td.style.padding=0;ae(th,ct(v.label));ae(tr,th);ae(C,tr);tr=ce("tr")}ae(td,P);break;case"select":var P=D=ce("select");P.name=v.id;if(v.size){P.size=v.size}if(v.disabled){P.disabled=true}if(v.multiple){P.multiple=true}for(var L=0,A=B.length;L<A;++L){var I=ce("option");I.value=B[L];ae(I,ct(v.options[B[L]]));ae(P,I)}ae(td,P);break;case"dynamic":td.colSpan=2;td.style.textAlign="left";td.style.padding=0;if(v.compute){(v.compute.bind(d,null,l[v.id],o,td,tr))()}ae(tr,td);ae(C,tr);D=td;break;case"checkbox":case"radio":var K=0;D=[];for(var L=0,A=B.length;L<A;++L){var H=ce("span"),P,J,w="sdfler46"+v.id+"-"+B[L];if(L>0&&!v.noInputBr){ae(td,ce("br"))}J=ce("label");J.setAttribute("for",w);J.onmousedown=rf;P=ce("input");P.setAttribute("type",v.type);P.name=v.id;P.value=B[L];P.id=w;if(v.disabled){P.disabled=true}if(v.submitOnDblClick){J.ondblclick=P.ondblclick=function(S){p()}}if(v.compute){(v.compute.bind(d,P,l[v.id],o,td,tr))()}ae(J,P);ae(J,ct(v.options[B[L]]));ae(td,J);D.push(P)}break;default:var P=D=ce("input");P.name=v.id;if(v.size){P.size=v.size}if(v.disabled){P.disabled=true}if(v.submitOnEnter){P.onkeypress=function(S){S=$E(S);if(S.keyCode==13){p()}}}P.setAttribute("type",v.type);ae(td,P);break}if(v.label){if(v.type=="textarea"){if(v.labelAlign){td.style.textAlign=v.labelAlign}td.colSpan=2}else{if(v.labelAlign){th.style.textAlign=v.labelAlign}ae(th,ct(v.label));ae(tr,th)}}if(v.type!="checkbox"&&v.type!="radio"){if(v.width){P.style.width=v.width}if(v.compute&&v.type!="caption"&&v.type!="dynamic"){(v.compute.bind(d,P,l[v.id],o,td,tr))()}}if(v.caption){var H=ce("small");if(v.type!="textarea"){H.style.paddingLeft="2px"}ae(H,ct(v.caption));ae(td,H)}ae(tr,td);ae(C,tr);z=v.mergeCell;b[v.id]=D}for(var M=i.buttons.length;M>0;--M){var F=i.buttons[M-1],Q=ce("a");Q.onclick=p.bind(Q,F[0]);Q.className="dialog-"+F[0];Q.href="javascript:;";ae(Q,ct(F[1]));ae(E,Q)}var R=ce("div");R.className="clear";ae(E,R);c=true}function n(){for(var z=0,u=i.fields.length;z<u;++z){var B=i.fields[z],A=b[B.id];switch(B.type){case"caption":break;case"select":for(var w=0,v=A.options.length;w<v;w++){A.options[w].selected=(A.options[w].value==l[B.id]||in_array(l[B.id],A.options[w].value)!=-1)}break;case"checkbox":case"radio":for(var w=0,v=A.length;w<v;w++){A[w].checked=(A[w].value==l[B.id]||in_array(l[B.id],A[w].value)!=-1)}break;default:A.value=l[B.id];break}if(B.update){(B.update.bind(d,null,l[B.id],o,A))()}}}function k(){if(i.onHide){i.onHide()}if(j.onHide){j.onHide()}}function p(v){if(v=="x"){return Lightbox.hide()}for(var w=0,u=i.fields.length;w<u;++w){var A=i.fields[w],z;switch(A.type){case"caption":continue;case"select":z=q(A.id);break;case"checkbox":case"radio":z=t(A.id);break;case"dynamic":if(A.getValue){z=A.getValue(A,l,o);break}default:z=h(A.id);break}if(A.validate){if(!A.validate(z,l,o)){return}}if(z&&typeof z=="string"){z=trim(z)}l[A.id]=z}g(v)}function g(v){var u;if(r){u=r(l,v,o)}if(j.onSubmit){u=j.onSubmit(l,v,o)}if(u===undefined||u){Lightbox.hide()}return false}function h(u){return b[u].value}function f(v,u){b[v].value=u}function q(A){var z=[],w=b[A];for(var v=0,u=w.options.length;v<u;v++){if(w.options[v].selected){z.push(parseInt(w.options[v].value)==w.options[v].value?parseInt(w.options[v].value):w.options[v].value)}}if(z.length==1){z=z[0]}return z}function t(A){var z=[],w=b[A];for(var v=0,u=w.length;v<u;v++){if(w[v].checked){z.push(parseInt(w[v].value)==w[v].value?parseInt(w[v].value):w[v].value)}}return z}};Dialog.templates={};
...
Listview.templates.forumspost = {
	...
	poundable: 2,
	columns: [],
	compute: function (q, W, V) {
		var e = (g_user.roles & 13) != 0, // Moderator Rights
		L = e || (g_pageInfo.locked == 0),
		D = L && !q.op,
		Y = 0,
		z = g_users[q.user],
		c = 0,
		...
		X = 0,
		w = null;
		/*if (g_pageInfo.blog && q.op && (g_user.roles & 152)) {
			if (q.user.toLowerCase() == g_user.name.toLowerCase()) {
				L = true
			} else {
				L = false
			}
		...
		if (z != null) {
			if (g_customColors[g_users[q.user].name]) {
				w = g_customColors[g_users[q.user].name]
			} else {
				if (z.roles & 12) {
					c = 1
				} else {
			...
			if ((z.roles & 8) == 0 && (!g_pageInfo.blog || !q.op)) {
				Y = 1
		...
		var o = ce("div");
		o.className = "forums-post-topcorners";
		ae(o, ce("div"));
		ae(o, ce("p"));
		ae(g, o);
		if (!this.blog) {
			var A = ce("div");
			A.className = "forums-post-userinfo";
			if (z != null) {
				if (z.avatar) {
					ae(A, Icon.createUser(z.avatar, z.avatarmore, 2, null, z.roles & 8/*Blogger?*/))
				}
			}
			var S = ce("p");
			var R = ce((z == null || z.level != null) ? "b" : "a");
			if (z != null && z.classs) {
				R.className = "c" + z.classs;
			}
			if (z != null && z.level == null) {
				R.href = "?user=" + g_users[q.user].name;
			}
			ae(R, ct(g_users[q.user].name));
			ae(S, R);
			...
			if (z != null) {
				if(z.posts)
				{
					ae(P, ct(LANG.lvtopic_posts + z.posts));
					//ae(P, ce("br"));
				}
				/* if(z.classs && z.race && z.level)
				{
					ae(P, ct(LANG.level + ": " + z.level));
					ae(P, ce("br"));
					ae(P, ct(LANG.classs + ": " + g_chr_classes[z.classs]));
					ae(P, ce("br"));
					ae(P, ct(LANG.race + ": " + g_chr_races[z.race]));
					ae(P, ce("br"));
				} */
				// Hide level, class, race (avatar ftw)
				if (z.roles > 0) {
					var K = 0;
					for (var T = 1; T <= 9; ++T) {
						if (z.roles & (1 << (T - 1))) {
					...
					if (K > 0) {
						ae(P, ce("br"));
						ae(P, ct((K != 1 ? LANG.lvtopic_roles : LANG.lvtopic_role)));
						K = 0;
						for (var T = 1; T <= 9; ++T) {
							if (z.roles & (1 << (T - 1))) {
								if (K++>0) {
		...
		if (Y) {
			...
			if (g_user.id > 0) {
				l.onclick = Listview.funcBox.coReportClick.bind(l, q, 1);
				l.href = "javascript:;"
			} else {
				l.href = "?account=signin" + "&next=" + g_types[g_pageInfo.type] + "=" + g_pageInfo.typeId;
			}
		...
		if (g_pageInfo.locked == 0 || e) {
			...
			if (g_user.id > 0) {
				O.href = "#reply";
				O.onclick = fo_goToNewPost.bind(O, 1, q)
			} else {
				O.href = "?account=signin" + "&next=" + g_types[g_pageInfo.type] + "=" + g_pageInfo.typeId;
			}
			...
			if (g_user.id > 0) {
				...
				n.href = "?account=signin" + "&next=" + g_types[g_pageInfo.type] + "=" + g_pageInfo.typeId;
			}
		...
		if (q.nda) {
			var _s = ce("a");
			_s.href = "#p" + q.nda;
			_s.onmouseover = function (d) {
				Tooltip.showAtCursor(d, LANG.bugtracker_devsanswer3, 0, 0, "q")
			};
			_s.onmousemove = Tooltip.cursorUpdate;
			_s.onmouseout = Tooltip.hide;
			_s.style.background = "url(templates/wowhead/images/blizz.gif) no-repeat left center";
			_s.style.paddingLeft = "26px";
			ae(_s, ct(" "));
			ae(d, _s);
		}
		ae(v, ct(LANG.lvtopic_by));
		R = ce((z == null || z.level != null) ? "b" : "a");
		if (z != null && z.classs) {
			R.className = "c" + z.classs;
		}
		if (z != null && z.level == null) {
		...
		a.className = "q0";
		a.href = "#p" + q.id;
		var B = new Date(q.date),
		Q = (g_serverTime - B) / 1000;
		Listview.funcBox.coFormatDate(a, Q, B, 1);
		a.style.cursor = "pointer";
		ae(v, a);
		ae(v, ct(LANG.lvcomment_patch1 + g_getPatchVersion(B) + LANG.lvcomment_patch2));
		if (this.blog && z != null && z.avatar) {
			var f = Icon.createUser(z.avatar, z.avatarmore, 0, null, z.roles & 256);
			if (!Browser.ie6) {
		...
		if (w) {
			...
			if (c) {
				...
				if (h) {
					...
					if (X) {
						u.className += " comment-olive"
					}
				}
			}
		}
		var K = Markup.CLASS_USER;
		if (z && z.roles) {
			K = Markup.rolesToClass(z.roles)
		}
		u.innerHTML = Markup.toHtml(q.body, {
			allow: K,
			roles: z ? z.roles : null,
			uid: q.id
		});
		ae(p, u);
		if (z == null || z.roles == 0 || g_user.roles > 0) {
			var N = ce("div");
			q.divLastEdit = N;
		...
		if (q.nwStatus != null || q.nwUrl != null || q.nwTitle != null || z.sig != null) {
			var C = ce("div");
			C.className = "forums-post-sig";
			if (trim(q.body) == '') {
				C.className += " forum-post-sig-noline";
			}
			if (w) {
			...
			if (q.nwStatus != null || q.nwUrl != null || q.nwTitle != null || z.sig != null) {
				...
				Tt.style.backgroundColor = "#141414";
				Tt.style.width = "400px";
				Tt.style.padding = "8px";
				Tt.style.styleFloat = Tt.style.cssFloat = "left";
				Tt.className = "rcorners text";
				ae(Tt, ce("tt"));
				ae(Tt, ce("strong"));
				ae(Tt, ce("var"));
				ae(Tt, ce("em"));
				/* if(q.nwStatus != null) {
					ae(Tt, ct(LANG.bugtracker_stchange));
					Ty = ce("span");
					ae(Ty, ct(LANG.bugreport_statuses[q.nwStatus]));
					g_descBugStatus(q.nwStatus, Ty, null);
					ae(Tt, Ty);
					ae(Tt, ce("br"));
				} */
				/* if(q.nwTitle != null) {
					ae(Tt, ct(LANG.bugtracker_titlechange));
					Ty = ce("b");
					ae(Ty, ct(q.nwTitle));
					ae(Tt, Ty);
					ae(Tt, ce("br"));
				...
				if(q.nwUrl != null) {
					ae(Tt, ct(LANG.bugtracker_urlchange));
					Ty = ce("span");
					if (q.nwUrl[0]) {
						Ty.innerHTML = Markup.toHtml("[" + g_types[q.nwUrl[0]] + "=" + q.nwUrl[1] + "]", { mode: Markup.MODE_ARTICLE })
					} else {
						Ty.innerHTML = "<b>" + LANG.lvcomment_deleted + "</b>";
					}
					ae(Tt, Ty);
				}
				ae(C, Tt);
		...
		ae(U, g);
		/* var x = ce("div");
		x.className = "forums-post-bottomcorners";
		ae(x, ce("div"));
		ae(x, ce("p"));
		ae(g, x); */
		ae(W, U)
...
Listview.templates.forumslist = {
	...
	{
		id: "da",
		name: LANG.bugtracker_devsanswer1,
		tooltip: LANG.bugtracker_devsanswer2,
		align: "center",
		value: "da",
		width: "3%",
		filtrable: 0,
		...
		compute: function (d, f) {
			if (d.da) {
				var b = ce("a");
				b.onmouseover = function (d) {
					Tooltip.showAtCursor(d, LANG.bugtracker_devsanswer2, 0, 0, "q")
				};
				b.onmousemove = Tooltip.cursorUpdate;
				b.onmouseout = Tooltip.hide;
				b.href = this.template.getItemLink(d)+ "#p" + d.da;
				var c = ce("img");
				c.src = "templates/wowhead/images/blizz.gif";
				ae(b, c);
				ae(f, b)
			}
		}
	},
	{
		...
		compute: function (d, e) {
			...
			{
				var a = ce("a");
				a.href = "?" + g_types[d.obj[0]] + "=" + d.obj[1];
				//a.className = "small";
				if (d.obj[2]) {
					a.className += " q" + d.obj[2]
				}
				if (d.obj[3] || d.obj[0] == 5) {
					a.className += " icontiny";
					if (d.obj[0] != 5) {
						a.style.backgroundImage = "url(images/icons/tiny/" + d.obj[3] + ".gif)";
					} else {
						a.style.backgroundImage = "url(images/icons/tiny/quest_start.gif)";
					}
				}
				ae(a, ct(d.obj[4]));
				ae(e, a)
				...
				m.href = this.template.getItemLink(d);
				b = ce("span");
				ae(b, ct(d.subject));
				(g_pageInfo.mode == 1 ? g_descBugStatus(d.status, b, null) :  g_TopicStatus(d.status, b, null, d.locked, c));
				ae(m, b);
				ae(e, m)
			}
		},
		getVisibleText: function (a) {
			return a.obj ? a.obj[4] : a.subject
		},
		sortFunc: function (d, c, e) {
			return strcmp(d.obj ? d.obj[4] : d.subject, c.obj ? c.obj[4] : c.subject)
		}
	},
	//TODO:
	//{id:"preview",name:LANG.preview,align:"left",width:"50%",value:"preview",compute:function(c,f){var e=ce("div");e.className="crop";ae(e,ct(Markup.removeTags(c.preview,{mode:Markup.MODE_ARTICLE,uid:c.id})));ae(f,e);if(c.user){e=ce("div");e.className="small3";ae(e,ct(LANG.lvtopic_by));var b=ce("a");b.href="/user="+c.user;ae(b,ct(c.user));ae(e,b);ae(f,e)}}},
	{
		id: "status",
		name: LANG.status,
		type: "text",
		...
		compute: function (d, e) {
			var b = ce("a");
			b.href = "?bugtracker&status=" + g_bugtracker_statuses[d.status];
			b.style.fontSize = "11px";
			ae(b, ct(LANG.bugreport_statuses[d.status]));
			(g_pageInfo.mode == 1 ? g_descBugStatus(d.status, b, null) :  g_TopicStatus(d.status, b, null, d.locked, d.subject));
			ae(e, b);
		},
		getVisibleText: function (a) {
			return LANG.bugreport_statuses[a.status]
		},
		sortFunc: function (d, c, e) {
			var a = bt_sortStatus(d.status),
				b = bt_sortStatus(c.status);
	...
	},
	{
		...
		compute: function (d, e) {
			...
			ae(a, ct(d.score));
			ae(e, a);
			if (g_user && !g_user.needsupdate && !g_pageInfo.cannotstare) {
				if(d.stared) {
					g_starBugreport(false, a, null, d.id)
				} else {
					g_unstarBugreport(false, a, null, d.id)
				}
			}
		}
	},
	...
	},
	{
		...
		compute: function (d, e) {
			e.className = "small q1";
			var a = ce("a");
			a.href = "?bugtracker&category=" + g_bugtracker_categories[d.category];
			ae(a, ct(LANG.bugtracker_categories[d.category]));
			ae(e, a);
		},
		getVisibleText: function (a) {
			return LANG.bugtracker_categories[a.category]
		}
	},
	{
		id:"replies",name:LANG.replies,value:"replies",width:"7%"
	},
	{
		id:"views",name:LANG.views,value:"views",width:"7%"
	},
	{
		id: "category2",
		name: LANG.subcategory,
		value: "category2",
		width: "14%",
		type: "text",
		compute: function (d, e) {
			if(LANG.bugtracker_categories2[d.category] && LANG.bugtracker_categories2[d.category][d.category2]) {
				e.className = "small";
				var a = ce("a");
				a.href = "javascript:;";
				a.className = LANG.bugtracker_categories2[d.category][d.category2][1];
				if(LANG.bugtracker_categories2[d.category][d.category2][2]) {
					a.className += " icontiny";
					a.href = "?bugtracker&category=spells:" + d.category2;
					a.style.backgroundImage = "url(images/icons/tiny/" + LANG.bugtracker_categories2[d.category][d.category2][2] + ".gif)"
				}
				ae(a, ct(LANG.bugtracker_categories2[d.category][d.category2][0]));
				ae(e, a);
			} else if (d.category == 1 && d.category2) {
				e.className = "small q1";
				var b = ce("a");
				b.href = "?bugtracker&category=quests:" + d.category2;
				ae(b, ct(g_quest_sorts[d.category2]));
				ae(e, b)
			}
		},
		getVisibleText: function (a) {
			if(LANG.bugtracker_categories2[a.category] && LANG.bugtracker_categories2[a.category][a.category2]) {
				return LANG.bugtracker_categories2[a.category][a.category2][0]
			} else if (a.category == 1 && a.category2) {
				return g_quest_sorts[a.category2]
			}
		}
	},
	{
		id: "dev",
		name: LANG.bugtracker_dev1,
		tooltip: LANG.bugtracker_dev2,
		width: "7%",
		type: "text",
		value: "dev",
		filtrable: 1,
	...
	},
	{
		...
		searchable: 0,
		compute: function (d, f) {
			var c = new Date(d.lastpost[0]),
			e = (g_serverTime - c) / 1000;
			f.className = "small";
			f.style.whiteSpace = "nowrap";
			ae(f, ct(LANG.lvboard_by));
			var b = ce(!d.lastpost[4] ? "a" : "span");
			b.style.fontWeight = "bold";
			if (!d.lastpost[4]) {
				b.href = "?user=" + d.lastpost[3]
			} else {
				b.className = "c" + d.lastpost[4]
			}
			...
			b.href = "?forums&topic=" + d.id + "#p" + d.lastpost[2];
			ae(f, b)
		}
	}],
	getItemLink: function (a) {
		if (g_pageInfo.mode != 0)
		{return "?bugtracker&issue=" + a.id}
		else
		{return "?forums&topic=" + a.id}
	}
};
var g_bugtracker_categories = ["items","quests","spells","npcs","misc","achievements"];
var g_bugtracker_statuses = ["new","accepted","closed","fixed","need-retest","closed","closed","closed","closed"];

var g_mTP = null;

function fo_addPageNavigation(e, d, f, b, a) {
	var c = {
		nItems: e,
		page: d,
		url: b,
...
function g_starBugreport(a, b, d, i) {
	var c = 0;
	if (!b) {
		b = ge('starredstatus');
		c = 1
	}
	b.style.background = "left center no-repeat";
	b.style.backgroundImage = "url(templates/wowhead/images/star.png)";
	b.style.paddingLeft = "20px";
	b.style.cursor = "pointer";
	if (!c) {
		b.onmouseover = function (d) {
			Tooltip.showAtCursor(d, LANG.bugtracker_starred, 0, 0, "q")
		};
		b.onmousemove = Tooltip.cursorUpdate;
		b.onmouseout = Tooltip.hide;
	} else {
		b.innerHTML = LANG.bugtracker_starred;
	}
	var f = 0;
	if (!d) {
		d = b;
	...
	d.onclick = g_unstarBugreport.bind(this, true, c ? null : b, f ? null : d, i);
	if(a) {
		new Ajax("?bugtracker&star=" + (i ? i : g_pageInfo.topic));
	}
	return false
}
function g_unstarBugreport(a, b, d, i) {
	var c = 0;
	if (!b) {
		b = ge('starredstatus');
		c = 1
	...
	b.style.background = "left center no-repeat";
	b.style.backgroundImage = "url(templates/wowhead/images/staroff.png)";
	b.style.paddingLeft = "20px";
	b.style.cursor = "pointer";
	if (!c) {
		b.onmouseover = function (d) {
			Tooltip.showAtCursor(d, LANG.bugtracker_notstarred, 0, 0, "q")
		};
		b.onmousemove = Tooltip.cursorUpdate;
		b.onmouseout = Tooltip.hide;
	} else {
		b.innerHTML = LANG.bugtracker_notstarred;
	}
	var f = 0;
	if (!d) {
		d = b;
	...
	d.onclick = g_starBugreport.bind(this, true, c ? null : b, f ? null : d, i);
	if(a) {
		new Ajax("?bugtracker&unstar=" + (i ? i : g_pageInfo.topic));
	}
	return false
}
// positions of statuses
function bt_sortStatus(i) {
	switch(i) {
		case 4: // STATUS_CHECK_AGAIN
			return 1;
		...
		case 2: // STATUS_NOT_A_BUG
		case 5: // STATUS_DUPLICATE
		case 6: // STATUS_INCORRECT
			return 4;
...
function g_descBugStatus(j, b, e) {
	switch(j)
	{
		// STATUS_NEW
		case 0:
		...
		case 6: // STATUS_INCORRECT
			if (e) {
				e.style.backgroundColor = "#240909";
			}
			b.style.fontStyle = "italic";
			b.className = "icon-locked q0";
			break;
		// STATUS_FIXED
		case 3:
			if (e) {
				e.style.backgroundColor = "#0A2409";
			}
			b.className = "icon-locked q2";
			b.style.fontWeight = "bold";
			break;
		// STATUS_CHECK_AGAIN
		case 4:
			b.className = "icon-sticky q1";
			b.style.fontWeight = "bold";
			break;
	}
}
function g_TopicStatus(j, b, e, l, s) {
	switch(j)
	{
		// TOPIC_NEW
		case 0:
		...
		// TOPIC_LOCKED
		case 1:
			b.style.backgroundColor = "#240909";
			b.style.fontStyle = "italic";
			b.className = "icon-locked q0";
			break;
		case 2: // TOPIC_STICKY
			b.className = "icon-sticky";
			break;
		case 3: // TOPIC_STICKY_2
			b.className = "icon-sticky2";
			break;
		// TOPIC_GREEN
		case 4:
	...
	if(l==1){b.className = "icon-locked q0";}
}
Listview.funcBox.foDelete = function (a) {
	if (confirm(LANG.confirm_deletepost)) {
		new Ajax("?forums&deletepost&id=" + a.id + "&topic=" + g_pageInfo.topic);
		this.deleteRows([a]);
...
Listview.funcBox.beforeTopics = function () {
	function a(g, f, h, e) {
		...
		} else { if (g == null || confirm("Are you sure you want to " + g + " " + (d.length == 1 ? "this topic" : "these " + d.length + " topics") + "?")) {
				var b = "",
				c = 0;
	...
	if (g_user.roles & 15) {
		...
		this.createCbControls = function (j) {
			...
			e.value = "Удалить";
			h.value = "Переименовать";
			c.value = "Соединить";
			g.value = "Lock \\ Unlock";
			e.onclick = a.bind(this, "delete", "deletetopic&id", 1, function (d) {
				if (!d.blog || (g_user.roles & 4)) {
					return 1
				}
			});
			c.onclick = (function () {
				var d = this.getCheckedRows();
				if (!d.length) {
					alert("Select bugreports to merge.");
				} else {
					var _ = [];
					for (var a in d) {
						_.push(d[a].id)
					}
					location.href = "?bugtracker&merge=" + _.join(",");
				}
			}).bind(this);
			g.onclick = a.bind(this, "lock/unlock", "locktopic&locked", 0, function (d) {
				if ((!d.blog || (g_user.roles & 4))) {
					d.locked = null;
					return 1
				}
				...
				if (!d.length) {
					alert("You might want to pick a topic to rename.")
						...
						alert("You may only rename one topic at a time.")
							...
							location.href = "?edit=blog&id=" + d[0].id
						} else {
			...
			i.onchange = (function (l) {
				...
				l.selectedIndex = 0;
				(a.bind(this, "move", "move-to-status=" + d + "&issues", 1, function (m) {
					if (!m.blog) {
						return 1
					}
				}))()
			...
			var f = ce("option");
			ae(f, ct("Выставить статус..."));
			f.style.color = "#bbbbbb";
			ae(i, f);
			for (var K in LANG.bugreport_statuses) {
				if ((K==1 || K==0 || K==4) && !(g_user.roles & 8)) {
					continue
				}
				f = ce("option");
				ae(f, ct(LANG.bugreport_statuses[K]));
				f.value = K;
				ae(i, f)
			}
			if (g_user.roles == 2) {
			...
			k.value = "Stick (не пашет)";
			b.value = "Unstick (не пашет)";
			k.onclick = a.bind(this, "stick", "sticktopic&sticky=1", 0, function (d) {
				if ((!d.blog || (g_user.roles & 8)) && !d.sticky) {
					d.sticky = 1;
					return 1
				}
			});
...
function fo_addOptBoards(b, g, f) {
	for (var e = 0, a = g.length; e < a; ++e) {
		if (g[e][0] == null) {
			if (e != 0) {
				break
...
function fo_validateForm(c, d) {
	...
	if (!Listview.funcBox.coValidate(a, 1)) {
		return false
	}
	if (g_user.permissions & 1) {
		return true
...
function fo_goToNewPost(d, b) {
	...
	if (c) {
		...
		if (d == 1 && b != null && b.user != null && b.body != null) {
			a.value = "[quote " + g_users[b.user].name + (g_users[b.user].roles == 2 ? " blizzard=true" : "") + "]" + b.body + "[/quote]\n\n"
		}
		setTimeout((function () {
			this.focus()
		}).bind(a), 25)
...
function fo_DeleteTopic() {
	if (confirm("Вы уверены, что хотите удалить этот багрепорт?")) {
		new Ajax("?forums=deletetopic&id=" + g_pageInfo.topic);
		alert(LANG.message_topicdeleted)
	}
}
...
g_numberFormat=function(a){x=(""+parseFloat(a)).split(".");a=x[0];x=x.length>1?"."+x[1]:"";if(a.length<=3){return a+x}return g_number_format(a.substr(0,a.length-3))+","+a.substr(a.length-3)+x};
aowow = {};
aowow.func = {};
aowow.account = {};
aowow.screenshot = {};
...
aowow.init = function(){
	...
	if(g_user.upload_status)
	{
		ge("upload-error").innerHTML = g_user.upload_status;
		new Announcement({ parent: 'announcement-1337', id: 1337, text: '[color=red][b]' + g_user.upload_status + '[/b][/color]', name: 'Upload Failed', style: 'padding-left: 350px;', status: 1, mode: 1 });
	}
	//Wowhead screenshot initial load
	if(wh_ss)
	{
		tabsRelated.setTabName(tabsRelated.tabs.length - 1, LANG.tab_wh_screenshots + " (" + LANG.tooltip_loading + ")");
		g_ajaxIshRequest("?screenshot=getwh&type=" + g_pageInfo.type + "&typeid=" + g_pageInfo.typeId);ge('infobox-sticky').innerHTML = '<span class="q10<br />">' + "<img src=\"templates/wowhead/images/loading3.gif\"></img>" +'</span>';
	}
	if(wh_cs)
	{
		tabsRelated.setTabName(tabsRelated.tabs.length - 2, LANG.tab_wh_comments + " (" + LANG.tooltip_loading + ")");
		g_ajaxIshRequest("?comment=getwh&type=" + g_pageInfo.type + "&typeid=" + g_pageInfo.typeId);
	}
	if(wh_model)
	{
		g_ajaxIshRequest("?plugin=getmodel&type=" + g_pageInfo.type + "&typeid=" + g_pageInfo.typeId + "&model=" + wh_model);
		//check = ge('modelv');
		//if(check)
		//ge('modelv').src = 'http://www.aowow.org/templates/wowhead/images/ajax_load_2.gif';
		//ge('modelv').height = "150";
		//ge('modelv').width = "150";
		//$fx('#modelv').fxAdd({type: 'width', from: 16, to: 150, step: 15, delay: 30}).fxHold(3000).fxRun();
		//$fx('#modelv').fxAdd({type: 'height', from: 11, to: 150, step: 15, delay: 30}).fxHold(3000).fxRun();
		//setTimeout(aowow.func.loadmodel, 3000);
		//ge('infobox-sticky').innerHTML = '<span class="q10<br />">' + "<img src=\"templates/wowhead/images/loading3.gif\"></img>" +'</span>';
	}
	//Filters
	if(filtered == '1')
	{
...
c = ge("inlink");
	if (c) {
		c.menu = [[0, "Ingame link", g_getIngameLink], [0, "Forum link", g_getForumLink]]
		c.menu.rightAligned = 1;
		c.onmouseover = Menu.show;
		c.onmouseout = Menu.hide
	}
...
aowow.func.loadmodel = function(){
	ge('modelv').src = 'http://www.aowow.org/images/models/' + g_types[g_pageInfo.type] + '/' + wh_model + '.png';
}
aowow.account.preview = function(){
	avatar = ge("avatar-name");
	preview = ge("avatar-preview");
	preview.innerHTML = "";
	preview.appendChild(Icon.create(avatar.value, 2, 2, 0));
	preview.appendChild(Icon.create(avatar.value, 1, 2, 0));
	preview.appendChild(Icon.create(avatar.value, 0, 2, 0));
}
aowow.account.avatar_changed = function(k){
a = ge("avatar-name");
r = ge("avatar-current");
c = ge("change-avatar");
//TODO LOCALE
if (k = true)
{
c.innerHTML = "Avatar changed!";
c.className = " text q2";
r.innerHTML = "";
r.appendChild(Icon.create(a.value, 2, 2, 0, 0, a.value, 'q2'));
}
}
aowow.account.sig_changed = function(){
s = ge("change-sig");
s.innerHTML = "Sig changed!";
s.className = " text q2";
}
aowow.account.setavatar = function(){
a = ge("avatar-name");
u = g_user.id;
ge("avatar-current").innerHTML = "";
ge("avatar-current").appendChild(Icon.create("", 2, 2, 0, 0, "Loading...", 'q10'));
new Ajax("?account=setavatar&user=" + u, {
						method: "POST",
						params: "avatar=" + urlencode(a.value),
						onSuccess: function(e){aowow.account.avatar_changed(e.responseText)}
					})
}
aowow.account.setsig = function(){
s = ge("sig-change");
a = gE(s, "textarea")[0];
u = g_user.id;
new Ajax("?account=setsig&user=" + u, {
						method: "POST",
						params: "sig=" + urlencode(a.value),
						onSuccess: aowow.account.sig_changed()
					})
}
aowow.account.recent = function(c){
ge("avatar-name").value = c;
}
aowow.account.set_buttons = function(){
whb = (ge("whb").checked == true ? '1' : '0');
wht = (ge("wht").checked == true ? '1' : '0');
wpb = (ge("wpb").checked == true ? '1' : '0');
tdb = (ge("3db").checked == true ? '1' : '0');
u = g_user.id;
new Ajax("?account=set_buttons&user=" + u, {
						method: "POST",
						params: "whb=" + whb + "&wht=" + wht + "&wpb=" + wpb + "&3db=" + tdb,
						onSuccess: aowow.account.sig_changed()
					})
}
aowow.account.set_3dplus = function(){
twodp = (ge("2dp").checked == true ? '1' : '0');
threedp = (ge("3dp").checked == true ? '1' : '0');
u = g_user.id;
new Ajax("?account=set_3dplus&user=" + u, {
						method: "POST",
						params: "2dp=" + twodp + "&3dp=" + threedp,
						onSuccess: aowow.account.sig_changed()
					})
}
aowow.revealtext = function(){
ge("book-generic").style.display = "";
ge("content").style.display = "";
}
aowow.plugin.getvotecost = function(a, b){
	ge("votecost").innerHTML = LANG.tooltip_loading;
	new Ajax("?plugin=fvc", {
						method: "POST",
						params: "name=" + urlencode(a) + "&id="+ urlencode(b),
						onSuccess: function(e){aowow.plugin.showvotecost(e.responseText)}
					})
}
aowow.plugin.showvotecost = function(a){
	ge("votecost").innerHTML = a;
	}
aowow.feedback.sent = function(){
	c = ge("feedback-connect");
	c.className = "icon-email";
	c.style.display = "";
	ae(c, ct("Feedback sent!"));
	$fx('#feedback-connect').fxAdd({type: 'opacity', from: 0, to: 100, step: 15, delay: 30}).fxRun();
	//$fx('#feedback-connect').fxAdd({type: 'opacity', from: 100, to: 0, step: 15, delay: 30}).fxHold(3000).fxRun();
	setTimeout(function(){c.innerHTML = "";}, 4000);
}
aowow.feedback.send = function(){
	un = ge("username-generic");
	te = ge("feedback-text");
	um = gE(te, "textarea")[0];
	new Ajax("?feedback=send&type=" + g_pageInfo.type + "&typeid=" + g_pageInfo.typeId, {
						method: "POST",
						params: "username=" + urlencode(un.value) + "&commentbody=" + urlencode(um.value) + (g_user.name ? "&user="+g_user.id : ""),
						onSuccess: aowow.feedback.sent()
					})
}
aowow.feedback.form = function(g){
...
q_colors = {0 : "ff9d9d9d", 1 : "ffffffff", 2 : "ff1eff00", 3 : "ff0070dd", 4 : "ffa335ee", 5: "ffff8000", 6 : "ffe5cc80"};
comments_sort = function(a){
	for (i=0,b=a.length;i<b;i++){if (a[i].rating <= 1) a.remove(i)}
}	
