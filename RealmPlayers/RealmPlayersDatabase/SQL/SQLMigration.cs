...
namespace VF
{
    public class SQLMigration
    {
        ...
        public static int LoadPlayer(string _PlayerName, WowRealm _Realm, SQLPlayerID _PlayerID /*= SQLPlayerID.Invalid()*/, out Player _ResultPlayer)
        {
            ...
            using (var comm = new SQLComm())
            {
                return VF.SQLMigration.LoadFullPlayer(comm, _PlayerName, _Realm, _PlayerID, out _ResultPlayerHistory, out _ResultPlayer, out _ResultExtraData);
            }
        }
        ...
        public static int LoadFullPlayer(VF.SQLComm _Comm, string _PlayerName, WowRealm _Realm, SQLPlayerID _PlayerID /*= SQLPlayerID.Invalid()*/, out PlayerHistory _ResultPlayerHistory, out Player _ResultPlayer, out ExtraData _ResultExtraData)
        {
                ...
                try
                {
                    ...
                    foreach (var player in playerIDs)
                    {
                        try
                        {
                            ...
                            int playerUpdateCount = LoadFullPlayer(comm, player.Key, _Realm, new SQLPlayerID(player.Value), out playerHistory, out playerData, out extraData);
